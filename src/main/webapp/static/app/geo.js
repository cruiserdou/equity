...
function change_edit(v) {
    ...
    with (ss) {
        length = 0;
        options[0] = new Option(opt0_edit[v], opt0_edit[v]);
        if (v && document.getElementById(s_edit[v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ar = dsy.Items[str];
                for (i = 0; i < ar.length; i++)options[length] = new Option(ar[i], ar[i]);
                if (v)options[0].selected = true;
...
function preselect_edit(p_key) {
    ...
    if (index < provinces.length) {
        document.getElementById(s_edit[0]).selectedIndex = index + 1;
        change_edit(1);
    }
}

var s_edit = ["province_edit", "city_edit", "county_edit"];
var opt0_edit = ["省份", "地级市", "市、县级市、县"];
function setup_edit() {
    for (i = 0; i < s_edit.length - 1; i++)
        document.getElementById(s_edit[i]).onchange = new Function("change_edit(" + (i + 1) + ");");
    change_edit(0);
}
